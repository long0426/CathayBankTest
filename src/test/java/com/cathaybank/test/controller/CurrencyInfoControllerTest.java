...
class CurrencyInfoControllerTest {
	...
	void testAddCurrencyInfo() throws Exception {
		mockMvc.perform(post("/currencyinfo/add")
				.content("{\"code\":\"TEST\",\"name\":\"測試\"}")
	...
	void testGetCurrencyInfo() throws Exception {
		mockMvc.perform(get("/currencyinfo/get?id=4")
				)
        .andExpect(status().isOk())
        .andDo(print());
	}
	...
	void testGetAllCurrencyInfo() throws Exception {
		mockMvc.perform(get("/currencyinfo/getall")
				)
        .andExpect(status().isOk())
        .andDo(print());
	}
	...
	void testModifyCurrencyInfo() throws Exception {
		mockMvc.perform(put("/currencyinfo/modify")
				.content("{\"id\":4,\"code\":\"GBP\",\"name\":\"測試\"}")
				.contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON_UTF8)
				)
        .andExpect(status().isOk())
	...
	void testDeleteCurrencyInfo() throws Exception {
		mockMvc.perform(delete("/currencyinfo/delete")
				.param("id", "4")
				)
        .andExpect(status().isOk())
        .andDo(print());
